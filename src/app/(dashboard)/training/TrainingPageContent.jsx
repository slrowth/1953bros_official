...
const CATEGORY_OPTIONS = ["전체", "운영 매뉴얼", "제품 정보", "서비스 가이드", "안전 관리", "기타"];
...
export default function TrainingPageContent() {
  ...
  useEffect(() => {
    if (isViewMode && selectedMaterial) {
      ...
      const hasLinkOnly =
        !hasDescription && mediaItems.length > 0 && mediaItems.every((item) => linkMediaTypes.includes(item.mediaType));
  ...
  if (!isViewMode) {
    return (
      <div className="flex flex-1 flex-col bg-neutral-50">
          ...
          {loading ? (
            ...
            <div className="rounded-2xl border border-neutral-200 bg-white p-12 text-center">
              ...
              <p className="mt-4 text-slate-500">
                {searchQuery || selectedCategory !== "전체" ? "검색 결과가 없습니다." : "등록된 교육 자료가 없습니다."}
              </p>
            </div>
          ) : (
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {filteredMaterials.map((material) => {
                ...
                const hasLinkOnly =
                  !hasDescription &&
                  mediaItems.length > 0 &&
                  mediaItems.every((item) => linkMediaTypes.includes(item.mediaType));
                const hasFileAttachments = mediaItems.some((item) => fileMediaTypes.includes(item.mediaType));

                const firstMediaType =
                  mediaItems.length > 0 ? mediaItems[0].mediaType : material.mediaType || "FILE";
                const Icon = getMediaTypeIcon(firstMediaType);

                return (
                  <div
                    key={material.id}
                    className="group cursor-pointer rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm transition hover:shadow-md"
                    onClick={() => {
                      // 링크만 있는 경우 바로 이동
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex flex-1 items-center gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <h3 className="font-semibold text-slate-900 transition hover:text-[#967d5a]">
                            {material.title}
                          </h3>
                    ...
                    {hasDescription && <p className="mt-3 line-clamp-2 text-sm text-slate-600">{material.description}</p>}
        ...
        {/* 문의하기 패널 (목록 화면) */}
        <InquiryPanel trainingMaterialId={null} isOpen={isInquiryOpenList} onClose={() => setIsInquiryOpenList(false)} />
      </div>
    );
  }
              ...
              {fetching ? (
                ...
                <div className="rounded-3xl bg-white p-8 shadow-sm">
                  ...
                  {selectedMaterial.description && (
                    <div className="mb-6">
                      <div className="prose max-w-none whitespace-pre-wrap text-slate-600">
                        {selectedMaterial.description}
                      </div>
                    </div>
                  )}
                  <div className="mb-6 flex items-center gap-4">
                    ...
                    {fileAttachments.length > 0 && (
                      <span className="text-sm text-slate-500">첨부파일 {fileAttachments.length}개</span>
                    )}
                          ...
                          >
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <p className="max-w-md truncate text-xs text-slate-500">{fileItem.mediaUrl}</p>
                              </div>
                            </div>
                            <a
